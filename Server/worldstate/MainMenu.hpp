		...
		void Handle (const char * buffer)
		{
			...
			switch (type)
			{
				...
				case RSREGISTER:
				{
					...
					m_server.CreateAccount(username, password, [this]()
					{
						Packet outgoing;
						outgoing.WriteUInt8(RCREGISTER);
				...
				case RSREQCHARLIST:
				{
					...
					for (Go* character : chars)  // use the getter
					{
					    ...
					    if (character)
					    {
					    	...
					    	outgoing.WriteString(character->Actor()->GetTitle());
				...
				case RSDELETECHAR:
				{
					int selectSlot = incoming.ReadUInt8();
					Account* account = rememberedAccount;
					godb.DeleteChar(selectSlot, account, [this]()
						{
							cout << "DeleteChar exec success " << endl;
							Packet outgoing;
				...
				case RSCREATECHAR:
				{
					...
					else if (account->GetCharacters().size() < (size_t)4)
					{
						...
						godb.CreateChar(account, charName, charType, charHead, charSkin, charHair, charShirt, charPants, [this]()
							{
								// TODO add entry to t_accounts
								cout << "CreateCharDB exec success " << endl;
								Packet outgoing;
								outgoing.WriteUInt8(RCCREATECHAR);
								outgoing.WriteUInt8(0); // success
								m_connection->Send(outgoing.Data(), outgoing.Size());
