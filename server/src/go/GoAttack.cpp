...
float GoAttack::CalcDamage(Go* target, const string& skill, int hitType, GoMagic* magic)
{
	...
	Go* attacker = GetGo();
	if (log) std::cout << "[CalcDamage] Got attacker: " << (attacker ? "valid" : "null") << std::endl;

	if (!target || !attacker) {
		if (log) std::cout << "[CalcDamage] Target or attacker is null. Exiting early." << std::endl;
		return 0.0f;
	}

	//GoAttack* atkComp = attacker->Attack();
	GoActor* actorComp = attacker->Actor();
	if (log) std::cout << "[CalcDamage] Got actor component: " << (actorComp ? "valid" : "null") << std::endl;

	Go* weapon = NULL; // Default: unarmed
	//if (log) std::cout << "[CalcDamage] Default weapon set to attacker." << std::endl;

	// Try to get the equipped weapon
	if (skill == "melee")
	{
		Go* equipped = attacker->Inventory()->GetEquipped(es_weapon_hand);
		if (log) std::cout << "[CalcDamage] Equipped melee weapon: " << (equipped ? "valid" : "null") << std::endl;
		if (equipped && equipped->HasAttack())
		{
			weapon = equipped;
			if (log) std::cout << "[CalcDamage] Melee weapon updated to equipped." << std::endl;
		}
	}
	else if (skill == "ranged")
	{
		Go* equipped = attacker->Inventory()->GetEquipped(es_shield_hand);
		if (log) std::cout << "[CalcDamage] Equipped ranged weapon: " << (equipped ? "valid" : "null") << std::endl;
		if (equipped && equipped->HasAttack())
		{
			weapon = equipped;
			if (log) std::cout << "[CalcDamage] Ranged weapon updated to equipped." << std::endl;
		}
	}
	...
	if (weapon && weapon->HasAttack())
	{
		...
		if (log) std::cout << "[CalcDamage] Weapon base damage: min=" << min_damage << ", max=" << max_damage << std::endl;
	}
	/*if (attacker != weapon && skill == "melee")
	{
		min_damage += attacker->Attack()->DamageMin();
		max_damage += attacker->Attack()->DamageMax();
		if (log) std::cout << "[CalcDamage] Added attacker's damage: min=" << min_damage << ", max=" << max_damage << std::endl;
	}*/
	...
	if (skill == "melee")
	{
		...
		if (actorComp && actorComp->CanLevelUp())
		{
			...
			if (log) std::cout << "[CalcDamage] Strength: " << strength << std::endl;
			...
			if (log) std::cout << "[CalcDamage] After strength bonus: min=" << min_damage << ", max=" << max_damage << std::endl;
		}
		else
		{
			//min_damage += atkComp->DamageBonusMinMelee();
	...
	else if (skill == "ranged")
	{
		...
		if (log) std::cout << "[CalcDamage] Skipping ranged bonus (commented out)." << std::endl;
	}
	else if (skill == "nature magic" || skill == "combat magic")
	{
		...
		if (log) std::cout << "[CalcDamage] Skipping magic damage logic (commented out)." << std::endl;
	}

	// Clamp and validate damage range
	min_damage = std::max(1.0f, min_damage);
	max_damage = std::max(min_damage, max_damage);
	if (log) std::cout << "[CalcDamage] Clamped damage range: min=" << min_damage << ", max=" << max_damage << std::endl;

	if (!engine.IsPlayer(attacker))
	{
		min_damage = attacker->Attack()->DamageMin();
		max_damage = attacker->Attack()->DamageMax();
	}
	...
	if (log) std::cout << "[CalcDamage] Rolled base damage: " << base_damage << std::endl;
	...
	if (log) std::cout << "[CalcDamage] Skill level: " << skillLevel << ", Defense: " << defense << std::endl;
	...
	else
	{
		...
		if (log) std::cout << "[CalcDamage] Skill type unrecognized, using fallback base damage." << std::endl;
	}

	cout << "Calculated damage: " << damage << endl;
	if (log) std::cout << "[CalcDamage] Damage before clamp: " << damage << std::endl;

	damage = ClampRelative(damage, preDefense);
	if (log) std::cout << "[CalcDamage] Final damage after ClampRelative: " << damage << std::endl;
...
uint64_t GoAttack::CalcHitTime()
{
	...
	if (engine.IsPlayer(go))
	{
		...
		{
			std::cout << "CalcHitTime attacker aspect not found" << std::endl;
			logger.WriteF("CalcHitTime attacker aspect not found");
			return defaultDelay;
		}
