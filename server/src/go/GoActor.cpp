...
static const std::vector<float> experience_table = {
...
float GoActor :: GetLevelFromXP(uint64_t xp) {
    ...
    uint64_t xp_start = table[level];
    uint64_t xp_end   = table[level + 1];
    float fraction = static_cast<float>(xp - xp_start) / (xp_end - xp_start);

    return static_cast<float>(level) + fraction;
}

uint64_t GoActor :: GetXPFromLevel(float level) {
    const auto& table = experience_table;
    ...
    uint64_t xp_start = table[baseLevel];
    uint64_t xp_end = table[baseLevel + 1];

    return static_cast<uint64_t>(xp_start + fraction * (xp_end - xp_start));
}
...
void GoActor :: AddSkillExp (const string & skill, float value)
{
	...
	if (std::floor(newLevel) > std::floor(oldLevel)) {
		if (skill != "uber") {
			engine.UpdateGoLvlup(GetGo(), skill);
