...
class SiegeCamera
{
	FEX void SetFieldOfViewDeg(float value)
	{
		float* addr = (float*)this;
		addr[0x49] = value * (3.14159265f / 180.0f);

		//addr[0x49] = value;
	}

	FEX float GetFieldOfViewDeg()
	{
		float* addr = (float*)this;
		return addr[0x49] * (180.0f / 3.14159265f);
	}
...
// Helper: load file into g_CustomVars map
static void LoadCustomVarsFromFile(const std::string& filename)
{
	g_CustomVars.clear();
	...
	while (std::getline(in, line))
	{
		...
		g_CustomVars[key] = val;
	}
}

// Helper: write g_CustomVars map to file
static void SaveCustomVarsToFile(const std::string& filename)
{
	...
	for (it = g_CustomVars.begin(); it != g_CustomVars.end(); ++it)
	{
		out << it->first << "=" << it->second << "\n";
	}
}

FEX void SaveCustomVar(const char* name, const char* value)
{
	const std::string filename = "customsave.txt";
	LoadCustomVarsFromFile(filename);

	// insert or overwrite
	g_CustomVars[name] = value;

	SaveCustomVarsToFile(filename);
}

FEX const char* ReadCustomVar(const char* name)
{
	const std::string filename = "customsave.txt";
	LoadCustomVarsFromFile(filename);

	std::map<std::string, std::string>::iterator it = g_CustomVars.find(name);
	if (it != g_CustomVars.end())
	{
		g_LastReadValue = it->second;
		return g_LastReadValue.c_str();
	}
